...
class ClockTimeWidget(Widget):
    ...
    def __init__(
        ...
        # Time text
        time_text: Div = Div(
            inner_content=(session_data or {}).get("time_string"),
            _id="clock",
...
class WeatherWidget(Widget):
    ...
    def weather_icon_alt(self: WeatherWidget, value: Any = None) -> str:
        weather_code = self._session_data["weather_code"]
        if weather_code is not None and weather_code in WEATHER_ICONS:
            weather_alt, _ = posixpath.splitext(
                posixpath.basename(WEATHER_ICONS[weather_code])
            )
...
class HomeScreen(Page):
    def __init__(self, session_data: Optional[Dict[str, Any]]):
        ...
        style = Link(rel="stylesheet", href=f"{CACHE_DIR}/css/carousel.css")
        # script for carousel
        script = Script(src=f"{CACHE_DIR}/js/carousel.js")
    ...
    def generate_carousel(self: HomeScreen) -> List[Div]:
        ...
        item1: Div = Div(
            Div(
                ...
                style={
                    "background-image": f"url('{CACHE_DIR}/images/pexels-photo-1631677.jpg')",
                },
            ),
            _class=item_classes,
        )
